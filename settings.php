...
/*
if (!drupal_is_cli() && (!stristr($_SERVER['HTTP_COOKIE'], 'SESS')) && ($_SERVER['REQUEST_URI'] <> '/user' && $_SERVER['REQUEST_URI'] <> '/user/register')) {
  $proto = $_SERVER['SERVER_PORT'] == 443 ? 'https' : 'http';
  $html = file_get_contents('/www/virtual/tcdrupal.org/drupal/sites/2014.tcdrupal.org/index.html');
  echo $html;
  flush();
  // header('Location: '. $proto . '://volunteer.mnartsboard.com/volunteer');
  die;
}
*/
...
$databases['default']['default'] = array(
  'driver' => '%DB_TYPE',
  'database' => '%DB_NAME',
  'username' => 'advantagelabs',
  'password' => 'onlineprogress',
  'host' => 'localhost',
  'prefix' => '',
);
...
// Append local settings such as debugging variables, alternative credentials.
if (file_exists($inc = dirname(__FILE__) .'/local.settings.inc')) require $inc;
